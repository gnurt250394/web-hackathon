...
import { updateListResult } from '../../../../utils/apiAxios'
...
class EditResult extends React.Component {
    ...
    create = () => {
        ...
        let data =
        {
             from_point, to_point, type, id
        }

        console.log('data: ', data);
        let token = this.props.userApp.currentUser.token
        updateListResult(data, token).then(res => {

            console.log(res)
            toast.success("Sửa kết quả thành công!", {
                position: toast.POSITION.TOP_RIGHT
            });
            this.handleClose();
                        ...
                        <DialogContent>
                            <Grid container spacing={16}>
                                <Grid item xs={12} md={12}>
                                    <Grid item xs={12} md={12}> <TextValidator
...
export default withStyles(styles)(connect(mapStateToProps)(EditResult));